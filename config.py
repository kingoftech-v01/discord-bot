...
# === TOKENS ===
# No defaults for secrets - bot will fail fast if not configured
...
SEND_HOUR = int(os.getenv("SEND_HOUR", "9"))  # UTC hour for scheduled sends
...
XP_COOLDOWN = int(os.getenv("XP_COOLDOWN", "60"))  # Seconds between XP gains
LEVEL_UP_BASE = int(os.getenv("LEVEL_UP_BASE", "100"))
LEVEL_UP_FACTOR = float(os.getenv("LEVEL_UP_FACTOR", "1.5"))  # Geometric scaling
...
# === MODERATION ===
WARN_THRESHOLD = int(os.getenv("WARN_THRESHOLD", "3"))  # Auto-mute after N warnings
MUTE_DURATION = int(os.getenv("MUTE_DURATION", "3600"))  # Seconds
SPAM_THRESHOLD = int(os.getenv("SPAM_THRESHOLD", "5"))
SPAM_INTERVAL = int(os.getenv("SPAM_INTERVAL", "5"))  # Rolling window seconds

# === PROFANITY FILTER ===
# Includes common bypass patterns (l33t, spacing). Per-guild words in database.
BANNED_WORDS = [
...
# === TRIVIA ===
TRIVIA_TIME_LIMIT = int(os.getenv("TRIVIA_TIME_LIMIT", "30"))
TRIVIA_REWARD = int(os.getenv("TRIVIA_REWARD", "50"))


class Colors:
    """Embed color constants for consistent UI."""
...
class Emojis:
    """Unicode emoji constants for bot messages."""
    SUCCESS = ""
    ERROR = ""
    WARNING = ""
    INFO = ""
